...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 26 14:43:54 2025
...
trf_matrx2 = make_drift(l5)* make_quad_defocus(f2) * make_drift(l4)* make_quad_focus(f1)* make_drift(l3)
drift_2 = make_drift(l2)
trfM2D2 = trf_matrx2*drift_2

vector_1 = sp.simplify(trfM2D2*make_kick_vector(kx1))
vector_2 = sp.simplify(trf_matrx2*make_kick_vector(kx2))

dummy_vector = sp.Matrix([0,1])
dummy_vec1 = sp.simplify(trfM2D2*dummy_vector)
dummy_vec2 = sp.simplify(trf_matrx2*dummy_vector)
dummy_vectorxxp = dummy_vec1 + dummy_vec2
kick_vector = sp.Matrix([kx1,kx2])
overall_matrix = sp.Matrix([[trfM2D2[0,1],trfM2D2[1,1]],
                            [trf_matrx2[0,1], trf_matrx2[1,1]]])

inverse_matrix = overall_matrix.inv()
simplified_inverse_matrix = sp.simplify(inverse_matrix)
pprint(simplified_inverse_matrix)
coord_vector = sp.Matrix([x,xp])
kicker_vector = sp.simplify(simplified_inverse_matrix*coord_vector)
...
def track_through_lattice(nodes, lattice, lattice_dict, kicker_names, end_index, x, xp):
    ...
    for n in nodes[:end_index+1]:
        ...
        if n.getName() in kicker_names:
            ...
            pprint(xxp)
            dummy_vector2 = kick_vector #stores the kick vector we currently are on
            
        else:
             accumulated_matrix =  lattice_dict[n.getName()]*accumulated_matrix #accumlates the TransferMatrix
    pprint(xxp)
    pprint(accumulated_matrix)
    xxp = accumulated_matrix * xxp
...
kicker_names = ["IKICKH_A10", "IKICKH_A11"]

xxp, trmatrixd5tod3, dummy_vector, dummy_vector2 = track_through_lattice(nodes, lattice, lattice_dict, kicker_names, 28, 0, 0)

trMatrix_1 = sp.Matrix([[dummy_vector[0]/kx1, 0],
                        [dummy_vector[1]/kx1, dummy_vector2[1]/kx2]])

trMatrix_total = trmatrixd5tod3*trMatrix_1
...
# def generate_transfer_matrix(lattice, nodes, lattice_dict, start_node_name):
#     total_matrix = sp.eye(2) #identity matrix to start 
#     dummy_matrix = sp.eye(2)
#     xxp = sp.Matrix([x,xp])
    
#     selected_nodes = nodes[:2 get_node_index(lattice, start_node_name) + 1]
#     reversed_node_sequence = selected_nodes[::-1]
    
#     for n in reversed_node_sequence:
        
#         if n.getType() != "kick teapot":
#             total_matrix = total_matrix * lattice_dict[n.getName()]
            
#         elif n.getType() == "kick teapot":
            
#             xxp == lattice_dict[n.getName()] + lattice_dict[n.getName()]
